...
path via the environment (DevOps §3).
"""
...
class EtlSettings(BaseSettings):
    ...
    Partition layout mirrors the design exactly (ETL §3):
    ...
    are incidents' (ETL §21)."""

    late_arrival_window_days: int = 35
    """How far back a nightly run reprocesses to absorb late data (ETL §6)."""

    volume_band_tolerance: float = 0.35
    """Fractional deviation from the trailing median that trips the volume
    check before an adaptive band has enough history (ETL §8)."""
    ...
    than silently using a stale rate (ETL §15)."""
